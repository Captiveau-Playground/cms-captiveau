...
const CTA: React.FC<CTAProps> = ({ marqueeItems = defaultMarqueeItems }) => {
    return (
        <section>
            <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 sm:py-20 py-8'>
                ...
                <div className="w-full py-4 bg-primary rounded-b-2xl overflow-hidden border-t border-black/5">
                    <Marquee className="[--duration:40s] [--gap:1.25rem] p-0" pauseOnHover>
                        {marqueeItems.map((item, index) => (
                            <div key={index} className="flex items-center gap-6">
                                ...
                                <Separator className="w-8! bg-white" />
                            </div>
                        ))}
                    </Marquee>
                </div>
