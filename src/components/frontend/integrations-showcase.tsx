...
import { FileSearch, Braces, Eye } from 'lucide-react'
...
/** Official brand marks (SVG paths) + brand colors. */
const brandPaths: Record<string, { path: string; hex: string }> = {
  ga4: {
    hex: '#E37400',
...
const meta: Record<
  IntegrationName,
  { label: string; description: string; icon?: React.ComponentType<{ className?: string; strokeWidth?: number }>; brand?: string }
> = {
  ga4: { label: 'Google Analytics 4', description: 'Konversi & engagement tracking', brand: 'ga4' },
  gsc: { label: 'Search Console', description: 'Indeks pencarian & performa query', brand: 'gsc' },
  clarity: { label: 'Microsoft Clarity', description: 'Heatmap & session recording', icon: Eye },
  gtm: { label: 'Tag Manager', description: 'Manajemen tag tanpa developer', brand: 'gtm' },
  'meta-pixel': { label: 'Meta Pixel', description: 'Retargeting & konversi iklan', brand: 'meta-pixel' },
  hotjar: { label: 'Hotjar', description: 'Behavioral insight & feedback', brand: 'hotjar' },
  seo: { label: 'SEO & Open Graph', description: 'Meta, OG, & social sharing', icon: FileSearch },
  schema: { label: 'Schema.org', description: 'Structured data untuk SERP', icon: Braces },
  performance: { label: 'Performance', description: 'Core Web Vitals & Lighthouse', brand: 'performance' },
}

function BrandMark({ brand, className }: { brand: string; className?: string }) {
  const data = brandPaths[brand]
  if (!data) return null
  return (
    <svg viewBox="0 0 24 24" fill={data.hex} aria-hidden="true" className={className}>
      <path d={data.path} />
    </svg>
  )
}
 ...
 * with each client project (GA4, GSC, Clarity, and more). Official brand marks.
 */
export default function IntegrationsShowcase({
  ...
  const items = integrations.map((name) => meta[name]).filter(Boolean)
          ...
          <div className="flex h-full flex-col gap-3 rounded-none border border-border bg-card p-4 transition-all duration-300 hover:border-primary/30 hover:shadow-lg hover:shadow-black/5">
            <span className="flex size-10 items-center justify-center rounded-none bg-muted">
              {item.brand ? (
                ...
                item.icon && <item.icon className="size-5 text-primary" strokeWidth={1.7} />
              )}
            </span>
