...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE IF NOT EXISTS \`services_key_benefits\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`icon\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`services_key_benefits_order_idx\` ON \`services_key_benefits\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`services_key_benefits_parent_id_idx\` ON \`services_key_benefits\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE IF NOT EXISTS \`services_process\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`step\` numeric,
  ...
  `)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`services_process_order_idx\` ON \`services_process\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`services_process_parent_id_idx\` ON \`services_process\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE IF NOT EXISTS \`services_usp\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`icon\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`services_usp_order_idx\` ON \`services_usp\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`services_usp_parent_id_idx\` ON \`services_usp\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE IF NOT EXISTS \`services_pricing_plans_basic_features\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`feature\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`services_pricing_plans_basic_features_order_idx\` ON \`services_pricing_plans_basic_features\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`services_pricing_plans_basic_features_parent_id_idx\` ON \`services_pricing_plans_basic_features\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE IF NOT EXISTS \`services_pricing_plans_best_deal_features\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`feature\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`services_pricing_plans_best_deal_features_order_idx\` ON \`services_pricing_plans_best_deal_features\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`services_pricing_plans_best_deal_features_parent_id_idx\` ON \`services_pricing_plans_best_deal_features\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE IF NOT EXISTS \`services_pricing_plans_enterprise_features\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`feature\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`services_pricing_plans_enterprise_features_order_idx\` ON \`services_pricing_plans_enterprise_features\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`services_pricing_plans_enterprise_features_parent_id_idx\` ON \`services_pricing_plans_enterprise_features\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE IF NOT EXISTS \`services_technologies\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`name\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`services_technologies_order_idx\` ON \`services_technologies\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`services_technologies_parent_id_idx\` ON \`services_technologies\` (\`_parent_id\`);`)
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX IF NOT EXISTS \`services_slug_idx\` ON \`services\` (\`slug\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`services_image_idx\` ON \`services\` (\`image_id\`);`)
  ...
  await db.run(sql`CREATE TABLE IF NOT EXISTS \`team_members_social_links\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`platform\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`team_members_social_links_order_idx\` ON \`team_members_social_links\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`team_members_social_links_parent_id_idx\` ON \`team_members_social_links\` (\`_parent_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`team_members_image_idx\` ON \`team_members\` (\`image_id\`);`)
  ...
  await db.run(sql`CREATE TABLE IF NOT EXISTS \`job_listings_requirements\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`requirement\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`job_listings_requirements_order_idx\` ON \`job_listings_requirements\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`job_listings_requirements_parent_id_idx\` ON \`job_listings_requirements\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE IF NOT EXISTS \`job_listings_benefits\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`benefit\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`job_listings_benefits_order_idx\` ON \`job_listings_benefits\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`job_listings_benefits_parent_id_idx\` ON \`job_listings_benefits\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE IF NOT EXISTS \`job_listings\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`title\` text NOT NULL,
  	\`department\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`job_listings_updated_at_idx\` ON \`job_listings\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`job_listings_created_at_idx\` ON \`job_listings\` (\`created_at\`);`)
  ...
  await db.run(sql`CREATE TABLE IF NOT EXISTS \`pages_blocks_hero_stats\` (
  	\`_order\` integer NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`pages_blocks_hero_stats_order_idx\` ON \`pages_blocks_hero_stats\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`pages_blocks_hero_stats_parent_id_idx\` ON \`pages_blocks_hero_stats\` (\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE TABLE IF NOT EXISTS \`pages_blocks_services_preview\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`pages_blocks_services_preview_order_idx\` ON \`pages_blocks_services_preview\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`pages_blocks_services_preview_parent_id_idx\` ON \`pages_blocks_services_preview\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`pages_blocks_services_preview_path_idx\` ON \`pages_blocks_services_preview\` (\`_path\`);`)
  await db.run(sql`CREATE TABLE IF NOT EXISTS \`pages_blocks_articles_preview\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`pages_blocks_articles_preview_order_idx\` ON \`pages_blocks_articles_preview\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`pages_blocks_articles_preview_parent_id_idx\` ON \`pages_blocks_articles_preview\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`pages_blocks_articles_preview_path_idx\` ON \`pages_blocks_articles_preview\` (\`_path\`);`)
  ...
  await db.run(sql`CREATE TABLE IF NOT EXISTS \`pages_blocks_rich_content\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`pages_blocks_rich_content_order_idx\` ON \`pages_blocks_rich_content\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`pages_blocks_rich_content_parent_id_idx\` ON \`pages_blocks_rich_content\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`pages_blocks_rich_content_path_idx\` ON \`pages_blocks_rich_content\` (\`_path\`);`)
  ...
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`pages_image_idx\` ON \`pages\` (\`image_id\`);`)
  ...
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`pages_rels_team_members_id_idx\` ON \`pages_rels\` (\`team_members_id\`);`)
  ...
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`payload_locked_documents_rels_job_listings_id_idx\` ON \`payload_locked_documents_rels\` (\`job_listings_id\`);`)
  ...
  await db.run(sql`CREATE TABLE IF NOT EXISTS \`site_settings_contacts\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`type\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`site_settings_contacts_order_idx\` ON \`site_settings_contacts\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`site_settings_contacts_parent_id_idx\` ON \`site_settings_contacts\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE IF NOT EXISTS \`site_settings_founders\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`name\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`site_settings_founders_order_idx\` ON \`site_settings_founders\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`site_settings_founders_parent_id_idx\` ON \`site_settings_founders\` (\`_parent_id\`);`)
